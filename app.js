(()=>{
  ...
  function renderHomeInsights(){
    ...
    root.innerHTML=`
      ...
      <article class="insight-card diagnostic"><span class="insight-icon">🎯</span><div><small>ĐÁNH GIÁ NĂNG LỰC</small><h3>${hasPre?'Đánh giá lại sau ôn tập':'Thiết lập đường cơ sở'}</h3><p>${hasPre?'So sánh với lần đánh giá đầu vào để thấy mức tiến bộ.':`15 câu phân bố theo ${course.levels.map(x=>esc(x)).join(' – ')}, dùng làm minh chứng trước khi ôn tập.`}</p><button class="text-btn" id="homeDiagnosticLink">${hasPre?'Đánh giá lại →':'Đánh giá đầu vào →'}</button></div></article>`;
  ...
  function startDiagnostic(){
    ...
    startQuiz(balancedQuestions(15),{mode:`diagnostic-${phase}`,diagnosticPhase:phase,title:hasPre?'Đánh giá lại năng lực':'Đánh giá đầu vào',timer:0});
  }
  ...
  function startQuiz(qs,opts){
    ...
    clearInterval(timerHandle);quiz={qs:[...qs],opts,index:0,answers:{},start:Date.now(),remaining:opts.timer||0,submitted:false};
    $('#quizTitle').textContent=opts.title;$('#quizStudent').textContent=`${profile.name} · ${profile.className} · Khối ${grade}`;renderQuestion();
    if(opts.timer){$('#quizTimer').textContent=`⏱ ${fmtTime(quiz.remaining)}`;timerHandle=setInterval(()=>{quiz.remaining--;$('#quizTimer').textContent=`⏱ ${fmtTime(quiz.remaining)}`;if(quiz.remaining<=0){clearInterval(timerHandle);finishQuiz();}},1000);}else $('#quizTimer').textContent='Không giới hạn';show('quiz');
  }
  function fmtTime(s){return `${String(Math.floor(s/60)).padStart(2,'0')}:${String(s%60).padStart(2,'0')}`;}
  function renderQuestion(){
    const q=quiz.qs[quiz.index],sel=quiz.answers[q.id],locked=quiz.opts.mode==='exam'||String(quiz.opts.mode).startsWith('diagnostic');
    $('#quizCounter').textContent=`Câu ${quiz.index+1}/${quiz.qs.length}`;$('#quizProgress').style.width=((quiz.index+1)/quiz.qs.length*100)+'%';
    $('#quizBody').innerHTML=`<div class="quiz-meta"><span>${esc(q.level)}</span><span>Bài ${q.lessonId}</span><span>Chủ đề ${q.topicId}</span></div><h2>${esc(q.question)}</h2><div class="options">${q.options.map((o,i)=>`<button class="option ${sel===i?'selected':''}" data-option="${i}"><b>${letters[i]}</b><span>${esc(o)}</span></button>`).join('')}</div><div id="feedback"></div><div class="quiz-actions"><button class="btn ghost" id="prevQ" ${quiz.index===0?'disabled':''}>← Trước</button><div>${!locked&&sel!==undefined?'<button class="btn ghost" id="checkQ">Xem giải thích</button> ':''}${quiz.index===quiz.qs.length-1?'<button class="btn primary" id="finishQ">Nộp bài</button>':'<button class="btn primary" id="nextQ">Tiếp →</button>'}</div></div>`;
    $('#quizBody').querySelectorAll('[data-option]').forEach(b=>b.onclick=()=>{quiz.answers[q.id]=+b.dataset.option;renderQuestion();if(!locked)showFeedback(q);});
    $('#prevQ').onclick=()=>{quiz.index--;renderQuestion()};if($('#nextQ'))$('#nextQ').onclick=()=>{quiz.index++;renderQuestion()};if($('#finishQ'))$('#finishQ').onclick=finishQuiz;if($('#checkQ'))$('#checkQ').onclick=()=>showFeedback(q);if(!locked&&sel!==undefined)showFeedback(q);
  }
  function showFeedback(q){
    const sel=quiz.answers[q.id];if(sel===undefined)return;const ok=sel===q.correctAnswer,f=$('#feedback');
    f.innerHTML=`<div class="feedback ${ok?'ok':'bad'}"><b>${ok?'✓ Chính xác':'✗ Chưa đúng'}</b><p>Đáp án: <span class="answer-key">${letters[q.correctAnswer]}. ${esc(q.options[q.correctAnswer])}</span></p><p>${esc(q.explanation)}</p></div>`;
    $('#quizBody').querySelectorAll('[data-option]').forEach((b,i)=>{b.classList.toggle('choice-correct',i===q.correctAnswer);b.classList.toggle('choice-wrong',i===sel&&!ok);});
  }
  ...
  function finishQuiz(){
    if(quiz.submitted)return;quiz.submitted=true;clearInterval(timerHandle);
